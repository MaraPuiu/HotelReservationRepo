...
function generateTable(container, initialHotels) {
	...
	function addOperationsCellToRow(row) {
        ...
        var hotelEditButton = $('<button/>', {
            'class': constants.ROW_EDIT_CLASS,
            'text': 'Edit'
        });
	...
	function initEvents() {
        ...
        container.on('click', constants.ROW_EDIT_CLASS.toClassFormat(), function () {
            onOperationEdit(this);
        });
    }
	
	function onOperationEdit(sender) {
    ...
    function enterEditMode(whereTo, hotelID) {
        var row = $('<tr/>', {
